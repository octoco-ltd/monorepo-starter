...
export const addHours = (firstDate: DateType | null, hours: number): Date => {
  ...
  if (typeof firstDate === 'number')
    return dayjs.unix(firstDate).add(hours, 'hours').toDate();
  return dayjs(firstDate).add(hours, 'hours').toDate();
};

export const addTime = (
  firstDate: DateType | null,
  amount: number,
  unit: 'hours' | 'milliseconds' | 'seconds' | 'minutes' | 'days'
): Date => {
  if (firstDate == null) return dayjs().add(amount, unit).toDate();
  if (typeof firstDate === 'number')
    return dayjs.unix(firstDate).add(amount, unit).toDate();
...
export const addMilliseconds = (
  ...
  if (typeof firstDate === 'number')
    return dayjs.unix(firstDate).add(milliseconds, 'milliseconds').toDate();
...
export const isToday = (date: DateType): boolean => {
  if (typeof date === 'number')
    return (
      dayjs.unix(date).format('DD MMM YYYY') === dayjs().format('DD MMM YYYY')
    );
...
export const isTomorrow = (date: DateType): boolean => {
  ...
  if (typeof date === 'number')
    return dayjs.unix(date).format('DD MMMM YYYY') === tomorrow;
